...
/**
 * Each of these is its own page in the sidebar now, so the component renders
 * exactly one and never shows a tab bar. Training is split by curriculum
 * category: every module is either "Sales" or "DM Sales".
 */
export type PlaybookView = "sops" | "sales-training" | "dm-training" | "messages";
...
// ─── Page ─────────────────────────────────────────────────────────────────────
const VIEW_META: Record<PlaybookView, { title: string; blurb: string }> = {
  "sops": { title: "Playbook", blurb: "Standard operating procedures for the team." },
  "sales-training": { title: "Sales Training", blurb: "The Scriptless Selling System, module by module." },
  "dm-training": { title: "DM Training", blurb: "Selling in the DMs, module by module." },
  "messages": { title: "Messages", blurb: "Message templates and video scripts, by moment in the funnel." },
};

export default function PlaybookWorkspace({ view = "sops" }: { view?: PlaybookView }) {
  const isTraining = view === "sales-training" || view === "dm-training";
  const trainingCategory = view === "dm-training" ? "DM Sales" : "Sales";

  // Training state
  const [completed, setCompleted] = useState<Set<string>>(new Set());
  // Open on the first lesson of THIS course. Seeding from CURRICULUM[0] put a
  // Sales lesson in the player on the DM Training page, which is the one thing
  // splitting the courses was meant to stop.
  const [activeLesson, setActiveLesson] = useState<TrainingLesson>(
    () => (CURRICULUM.find((m) => m.category === (view === "dm-training" ? "DM Sales" : "Sales")) ?? CURRICULUM[0]).lessons[0]
  );
  ...
  // Each training page owns its half of the curriculum, so progress and the
  // prev/next arrows stay inside it. A shared counter would tell someone on DM
  // Training they were 40% done because of Sales lessons they never opened.
  const courseModules = CURRICULUM.filter((m) => m.category === trainingCategory);
  const courseLessons = courseModules.flatMap((m) => m.lessons);
  const courseTotal = courseLessons.length;
  const completedCount = [...completed].filter((id) => courseLessons.some((l) => l.id === id)).length;
  const progressPct = courseTotal === 0 ? 0 : Math.round((completedCount / courseTotal) * 100);
  ...
  return (
    <div className="max-w-6xl mx-auto">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white tracking-tight">{VIEW_META[view].title}</h1>
        <p className="text-sm text-zinc-500 mt-1">{VIEW_META[view].blurb}</p>
      </div>
        ...
        <div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 mb-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h2 className="text-white font-bold text-lg">{view === "dm-training" ? "DM Sales" : "The Scriptless Selling System"}</h2>
                <p className="text-zinc-500 text-sm mt-0.5">{completedCount} of {courseTotal} lessons complete</p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-4 items-start">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden md:sticky md:top-20">
              <div className="divide-y divide-zinc-800/60">
                {(() => {
                  const categories = [...new Set(courseModules.map((m) => m.category))];
                  return categories.map((cat) => {
                    const catModules = courseModules.filter((m) => m.category === cat);
                    return (
                      <div key={cat}>
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center justify-between mt-4">
                <button
                  onClick={() => {
                    const prev = courseLessons[courseLessons.findIndex((l) => l.id === activeLesson.id) - 1];
                    if (prev) setActiveLesson(prev);
                  }}
                  disabled={courseLessons[0]?.id === activeLesson.id}
                ...
                <button
                  onClick={() => {
                    const next = courseLessons[courseLessons.findIndex((l) => l.id === activeLesson.id) + 1];
                    if (next) { toggleComplete(activeLesson.id); setActiveLesson(next); }
                  }}
                  disabled={courseLessons[courseLessons.length - 1]?.id === activeLesson.id}
