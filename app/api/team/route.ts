...
import { db, currentMember, hashPassword, tempPassword, logActivity, teamAccountMutationAllowed, type TeamMember } from "@/lib/team-auth";
...
export async function PATCH(req: NextRequest) {
  ...
  if (!me.active) return NextResponse.json({ error: "This account is inactive." }, { status: 403 });
  ...
  if (!teamAccountMutationAllowed(me, b.id, Object.keys(clean))) {
    return NextResponse.json({ error: "Only an active owner can change roles, account status, or other teammates." }, { status: 403 });
  }
